(() => {
  ...
  if (!product) {
    root.innerHTML = `
      <div class="pageShell">
        ...
        <p style="text-align:center"><a class="anda-btn" style="width:auto;display:inline-flex" href="${ANDA.routes ? ANDA.routes.categories() : "categories.html"}">カテゴリー一覧へ</a></p>
      </div>`;
    return;
  }
  ...
  root.innerHTML = `
      <nav class="breadcrumb" aria-label="パンくず">
        <a href="${ANDA.routes ? ANDA.routes.home() : "index.html"}">トップ</a><span>/</span>
        <a href="${ANDA.routes ? ANDA.routes.categories() : "categories.html"}">カテゴリー</a><span>/</span>
        <a href="${ANDA.routes ? ANDA.routes.catalog(product.category) : `catalog.html?cat=${product.category}`}">${category?.name || ""}</a><span>/</span>
      ...
      <div class="product-detail">
        ...
        <div>
          <p class="anda-product-card_cat"><a href="${ANDA.routes ? ANDA.routes.catalog(product.category) : `catalog.html?cat=${product.category}`}">${category?.name || ""}</a></p>
